...
export function ProductForm({ product }: ProductFormProps) {
  ...
  const [pdfFileName, setPdfFileName] = useState(product?.pdfUrl?.split('/').pop() || '');
  const [highlights, setHighlights] = useState<string[]>(product?.highlights || ['']);
  const [price, setPrice] = useState(product?.priceCents ? (product.priceCents / 100).toString() : '');
  ...
  return (
    <form action={handleSubmit} className="space-y-6 max-w-3xl">
      {state?.error && (
        <div className="rounded-md bg-red-50 p-3 text-sm text-red-800 dark:bg-red-900/20 dark:text-red-400">{state.error}</div>
      )}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input 
              ...
              className="w-full h-10 rounded-md border border-border bg-muted px-3 text-sm text-muted-foreground cursor-not-allowed" 
            />
            <p className="text-xs text-muted-foreground mt-1">Auto-filled based on type</p>
          </div>
          <div>
            ...
            <input 
              ...
              className="w-full h-10 rounded-md border border-border bg-background px-3 text-sm" 
            />
            <p className="text-xs text-muted-foreground mt-1">Will be displayed as KSh {price ? parseFloat(price).toLocaleString() : '0'}</p>
          </div>
        </div>
      ...
      <div className="rounded-lg border border-border/70 bg-card p-6 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {[0, 1, 2].map((index) => (
            <div key={index}>
              <label className="block text-sm font-medium mb-2">Image {index + 1} {index === 0 && '*'}</label>
              <ImageUpload
